...
public class ValidateDataTypeService {
	...
	public String validatePhone(Phone phone) {
		...
		if (!isEmpty(phone.getCountryCode()) && !isNumber(phone.getCountryCode())) {
			message = "El codigo de pais [countryCode] debe ser numerico";
		}

		if (!isEmpty(phone.getCityCode()) && !isNumber(phone.getCityCode())) {
			String messageCityCode = "El codigo de ciudad [cityCode] debe ser numerico";
			message = isEmpty(message) ? messageCityCode : message.concat("; ").concat(messageCityCode);
		}

		if (!isEmpty(phone.getNumber()) && !isNumber(phone.getNumber())) {
			String messageNumber = "El numero [number] debe ser numerico";
			message = isEmpty(message) ? messageNumber : message.concat("; ").concat(messageNumber);
		}
		return message;
	}
